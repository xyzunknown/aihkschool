...
export function BasicInfoSection({ school }: BasicInfoSectionProps) {
  ...
  const fields = [
    ...
    {
      label: "官方網站",
      value: school.website ? <a href={school.website} target="_blank" rel="noopener noreferrer" className="text-ink-900 hover:underline">{school.website}</a> : null,
    },
    {
      label: "聯絡電話",
  ...
  return (
    <section className="mb-8">
      <h2 className="text-xl font-semibold text-ink-900 mb-4">學校概況</h2>
      <GlassCard>
        <div className="space-y-4">
          {displayedFields.map((field) => (
            <div key={field.label} className="flex flex-col">
              <span className="text-xs font-medium text-ink-500 uppercase tracking-wide mb-1">
                {field.label}
              </span>
              <div className="text-base text-ink-900">{field.value}</div>
            </div>
          ))}
        </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {schoolandFields.map((field) => (
                <div key={field.label} className="flex flex-col">
                  ...
                  <div className="text-sm text-ink-900">{field.value}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {(school.schooland_intro || school.schooland_teaching_summary || school.schooland_facilities_summary) && (
          <div className="mt-5 border-t border-surface-border pt-5 space-y-4">
            {school.schooland_intro && (
              <div>
                <h3 className="text-sm font-semibold text-ink-800 mb-2">教學理念及課程</h3>
                <p className="text-sm text-ink-700 leading-relaxed">{school.schooland_intro}</p>
              </div>
            )}
            {school.schooland_teaching_summary && (
              <div>
                <h3 className="text-sm font-semibold text-ink-800 mb-2">教學情況</h3>
                <p className="text-sm text-ink-700 leading-relaxed">{school.schooland_teaching_summary}</p>
              </div>
            )}
            {school.schooland_facilities_summary && (
              <div>
                <h3 className="text-sm font-semibold text-ink-800 mb-2">學校設施</h3>
                <p className="text-sm text-ink-700 leading-relaxed">{school.schooland_facilities_summary}</p>
              </div>
            )}
          </div>
        )}

        <div className="mt-5 overflow-hidden rounded-card border border-surface-border bg-cream-100">
          {mapEmbedSrc ? (
            <iframe
            ...
            <div className="flex aspect-[4/3] items-center justify-center px-6 text-center text-sm text-ink-500">
              暫時未有足夠位置資料顯示地圖
            </div>
          )}
          ...
          {mapSearchHref && (
            <div className="flex items-center justify-between gap-3 border-t border-surface-border bg-white px-4 py-3">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="shrink-0 text-sm font-medium text-ink-700 underline hover:text-ink-900"
              >
                在 Google 地圖中開啟
              </a>
            </div>
