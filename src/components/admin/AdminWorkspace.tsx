...
const toneClasses: Record<Tone, string> = {
  neutral: "border-surface-border bg-cream-50 text-ink-700",
  good: "border-emerald-200 bg-forest-50 text-forest-700",
  warn: "border-amber-200 bg-amber-50 text-amber-700",
  danger: "border-red-200 bg-red-50 text-red-700",
};

const inputClasses =
  "min-h-10 w-full rounded-chip border border-surface-border bg-white px-3 py-2 text-sm text-ink-900 outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-100 disabled:bg-cream-50 disabled:text-ink-500";
...
export function AdminDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-ink-900/30">
      <div className="ml-auto flex h-full w-full max-w-3xl flex-col border-l border-surface-border bg-white shadow-2xl">
        <div className="border-b border-surface-border px-6 py-5">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              {eyebrow ? <p className="text-xs font-semibold uppercase tracking-wide text-ink-500">{eyebrow}</p> : null}
              <h2 className="mt-1 truncate text-xl font-bold text-ink-900">{title}</h2>
              {description ? <p className="mt-2 text-sm leading-6 text-ink-500">{description}</p> : null}
            </div>
            <button onClick={onClose} className="rounded-chip border border-surface-border px-3 py-2 text-sm font-medium text-ink-700 hover:bg-cream-50">
              關閉
            </button>
          </div>
        </div>
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">{children}</div>
        <div className="border-t border-surface-border bg-cream-50 px-6 py-4">
  ...
  return (
    <section className="mb-6">
      <div className="mb-3">
        ...
        {description ? <p className="mt-1 text-xs leading-5 text-ink-500">{description}</p> : null}
      </div>
      {children}
    </section>
  );
...
export function AdminCheckbox({ label, checked, onChange }: { label: string; checked: boolean; onChange: (value: boolean) => void }) {
  return (
    <label className="flex min-h-10 items-center gap-2 rounded-chip border border-surface-border bg-cream-50 px-3 text-sm text-ink-700">
...
export function AdminPublishChannels({
  ...
  return (
    <div className="md:col-span-2">
      <span className="mb-2 block text-xs font-medium text-ink-500">顯示在哪些端</span>
...
export function AdminInfoItem({ label, value, full = false }: { label: string; value: ReactNode; full?: boolean }) {
  return (
    <div className={full ? "md:col-span-2" : undefined}>
      ...
      <div className="mt-1 break-words text-sm font-medium leading-6 text-ink-800">{value || "未填寫"}</div>
    </div>
  );
}
