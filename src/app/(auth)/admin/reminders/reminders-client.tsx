  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-950">提醒狀態</h1>
          <p className="mt-1 text-sm text-slate-500">查看學校申請提醒和康體通開報提醒，並處理失敗項。</p>
        </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map((row) => (
              <tr key={row.id}>
                <td className="px-4 py-3 font-medium text-slate-950">{nameFor(row)}</td>
                <td className="px-4 py-3 text-slate-600">{row.users?.email ?? row.user?.email ?? "-"}</td>
                <td className="px-4 py-3 text-slate-600">{row.scheduled_date ?? row.scheduled_at ?? "-"}</td>
                <td className="px-4 py-3">{row.reminder_status ?? row.status}</td>
                <td className="px-4 py-3">{row.retry_count ?? 0}</td>
                <td className="px-4 py-3 text-right">
                  {(row.reminder_status ?? row.status) === "failed" ? <button onClick={() => act(row.id, "resend")} className="mr-4 font-medium text-slate-950 underline">重發</button> : null}
            ...
            {rows.length === 0 ? <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-400">沒有相關提醒</td></tr> : null}
          </tbody>
        </table>
      </div>
    </div>
