...
export function AdminUsersClient() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-950">用戶管理</h1>
        <p className="mt-1 text-sm text-slate-500">查看用戶收藏、課程追蹤、提醒記錄，並可停用異常用戶。</p>
      </div>
      <div className="mb-4 flex gap-3 rounded-2xl border border-slate-200 bg-white p-4">
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜尋 email / 名稱" className="min-w-0 flex-1 rounded-xl border border-slate-200 px-3 py-2 text-sm" />
        <button onClick={loadRows} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium">搜尋</button>
      </div>

      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-50 text-xs text-slate-500"><tr><th className="px-4 py-3">用戶</th><th className="px-4 py-3">收藏</th><th className="px-4 py-3">課程追蹤</th><th className="px-4 py-3">狀態</th><th className="px-4 py-3 text-right">操作</th></tr></thead>
          <tbody className="divide-y divide-slate-100">
            {rows.map((row) => (
              <tr key={row.id}>
                <td className="px-4 py-3 font-medium text-slate-950">{row.email}<div className="text-xs text-slate-400">{row.display_name ?? "-"} · {row.created_at.slice(0, 10)}</div></td>
                ...
                <td className="px-4 py-3 text-right"><button onClick={() => openDetail(row.id)} className="font-medium text-slate-950 underline">查看</button></td>
              </tr>
            ))}
      ...
      {detail && selectedUser ? (
        <AdminDrawer
          ...
          actions={<button onClick={() => toggleUser(selectedUser)} className="rounded-lg bg-slate-950 px-5 py-2 text-sm font-medium text-white">{selectedUser.admin_disabled ? "恢復用戶" : "停用用戶"}</button>}
        >
...
function Panel({ title, rows, labelKey }: { title: string; rows: unknown[]; labelKey: string }) {
  return <section className="rounded-lg border border-slate-200"><h3 className="border-b border-slate-100 px-4 py-3 text-sm font-semibold text-slate-950">{title}</h3><div className="max-h-80 divide-y divide-slate-100 overflow-auto">{rows.length ? rows.map((raw, index) => {
    ...
    return <div key={index} className="px-4 py-3 text-sm text-slate-700">{label}</div>;
  }) : <div className="px-4 py-8 text-center text-sm text-slate-400">沒有記錄</div>}</div></section>;
}
