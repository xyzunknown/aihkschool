  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-ink-900">面試情報審核</h1>
          <p className="mt-1 text-sm text-ink-500">審核、拒絕、隱藏和查找家長投稿。</p>
        </div>
      ...
      <div className="mb-4 grid gap-3 rounded-card border border-surface-border bg-white p-4 md:grid-cols-4">
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜尋學校" className="rounded-button border border-surface-border px-3 py-2 text-sm md:col-span-2" />
        <button onClick={load} className="rounded-button border border-surface-border px-3 py-2 text-sm font-medium">搜尋</button>
      </div>
      ...
      <div className="overflow-hidden rounded-card border border-surface-border bg-white">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {items.map((item) => (
              <tr key={item.id}>
                <td className="px-4 py-3 font-medium text-ink-900">{item.schools?.name_tc ?? "未知學校"}</td>
                <td className="px-4 py-3">{item.academic_year} · {item.grade_applied}</td>
                <td className="px-4 py-3">{item.application_result}</td>
                <td className="px-4 py-3 text-ink-700">{item.users?.email ?? "-"}</td>
                ...
                <td className="px-4 py-3 text-right"><button onClick={() => setSelected(item)} className="font-medium text-ink-900 underline">查看</button></td>
              </tr>
            ))}
          ...
          actions={
            <>
              {selected.status === "approved" ? (
                <button onClick={() => update(selected.id, { is_hidden: !selected.is_hidden })} className="rounded-chip border border-surface-border bg-white px-5 py-2 text-sm">{selected.is_hidden ? "取消隱藏" : "隱藏"}</button>
              ) : null}
              <button onClick={() => update(selected.id, { status: "rejected", rejectionReason: reason })} className="rounded-chip border border-red-200 bg-white px-5 py-2 text-sm text-red-700">拒絕</button>
              <button onClick={() => update(selected.id, { status: "approved", is_hidden: false })} className="rounded-chip bg-ink-900 px-5 py-2 text-sm font-medium text-white">通過</button>
            </>
          }
        >
          <AdminSection title="投稿內容">
            ...
            <div className="mt-4 whitespace-pre-line rounded-chip bg-cream-50 p-4 text-sm leading-6 text-ink-700">{selected.notes || "沒有備註"}</div>
          </AdminSection>
